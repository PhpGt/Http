...
use Gt\Http\Url;
use Gt\Http\UrlFactory;
use Gt\Http\UrlParseErrorException;
use PHPUnit\Framework\TestCase;
use TypeError;

class UriTest extends TestCase {
	public function testParsesProvidedUri() {
		$uri = new Url('https://user:pass@example.com:8080/path/123?q=abc#test');
	...
	public function testFromParts($input) {
		$uri = (new UrlFactory())->createFromParts(parse_url($input));
		self::assertSame($input, (string)$uri);
	}
	...
	public function testInvalidUrisThrowException($invalidUri) {
		self::expectException(UrlParseErrorException::class);
		new Url($invalidUri);
	}
	...
	public function testFragmentMustHaveCorrectType() {
		self::expectException(TypeError::class);
		(new Url())->withFragment([]);
	}

	public function testCanParseFalseyUriParts() {
		$uri = new Url('0://0:0@0/0?0#0');
	...
	public function testIsAbsolute() {
		$this->assertTrue((new Url('http://example.org'))->isAbsolute());
		$this->assertFalse((new Url('//example.org'))->isAbsolute());
		$this->assertFalse((new Url('/abs-path'))->isAbsolute());
		$this->assertFalse((new Url('rel-path'))->isAbsolute());
	}

	public function testIsNetworkPathReference() {
		$this->assertFalse((new Url('http://example.org'))->isNetworkPathReference());
		$this->assertTrue((new Url('//example.org'))->isNetworkPathReference());
		$this->assertFalse((new Url('/abs-path'))->isNetworkPathReference());
		$this->assertFalse((new Url('rel-path'))->isNetworkPathReference());
	}

	public function testIsAbsolutePathReference() {
		$this->assertFalse((new Url('http://example.org'))->isAbsolutePathReference());
		$this->assertFalse((new Url('//example.org'))->isAbsolutePathReference());
		$this->assertTrue((new Url('/abs-path'))->isAbsolutePathReference());
		$this->assertTrue((new Url('/'))->isAbsolutePathReference());
		$this->assertFalse((new Url('rel-path'))->isAbsolutePathReference());
	}

	public function testIsRelativePathReference() {
		$this->assertFalse((new Url('http://example.org'))->isRelativePathReference());
		$this->assertFalse((new Url('//example.org'))->isRelativePathReference());
		$this->assertFalse((new Url('/abs-path'))->isRelativePathReference());
		$this->assertTrue((new Url('rel-path'))->isRelativePathReference());
		$this->assertTrue((new Url(''))->isRelativePathReference());
	}

	public function testIsSameDocumentReference() {
		$this->assertFalse((new Url('http://example.org'))->isSameDocumentReference());
		$this->assertFalse((new Url('//example.org'))->isSameDocumentReference());
		$this->assertFalse((new Url('/abs-path'))->isSameDocumentReference());
		$this->assertFalse((new Url('rel-path'))->isSameDocumentReference());
		$this->assertFalse((new Url('?query'))->isSameDocumentReference());
		$this->assertTrue((new Url(''))->isSameDocumentReference());
		$this->assertTrue((new Url('#fragment'))->isSameDocumentReference());
		$baseUri = new Url('http://example.org/path?foo=bar');
		$this->assertTrue((new Url('#fragment'))->isSameDocumentReference($baseUri));
		$this->assertTrue((new Url('?foo=bar#fragment'))->isSameDocumentReference($baseUri));
		$this->assertTrue((new Url('/path?foo=bar#fragment'))->isSameDocumentReference($baseUri));
		$this->assertTrue((new Url('path?foo=bar#fragment'))->isSameDocumentReference($baseUri));
		$this->assertTrue((new Url('//example.org/path?foo=bar#fragment'))->isSameDocumentReference($baseUri));
		$this->assertTrue((new Url('http://example.org/path?foo=bar#fragment'))->isSameDocumentReference($baseUri));
		$this->assertFalse((new Url('https://example.org/path?foo=bar'))->isSameDocumentReference($baseUri));
		$this->assertFalse((new Url('http://example.com/path?foo=bar'))->isSameDocumentReference($baseUri));
		$this->assertFalse((new Url('http://example.org/'))->isSameDocumentReference($baseUri));
		$this->assertFalse((new Url('http://example.org'))->isSameDocumentReference($baseUri));
		$this->assertFalse((new Url('urn:/path'))->isSameDocumentReference(new Url('urn://example.com/path')));
	}
	...
	public function testWithoutQueryValueRemovesAllSameKeys() {
		$uri = (new Url())->withQuery('a=b&c=d&a=e');
		$uri = $uri->withoutQueryValue('a');
	...
	public function testWithoutQueryValueHandlesEncoding() {
		...
		$uri = (new Url())->withQuery('E%3dmc%5E2=einstein&foo=bar');
	...
	public function testSchemeIsNormalizedToLowercase() {
		...
		$uri = (new Url('//example.com'))->withScheme('HTTP');
		$this->assertSame('http', $uri->getScheme());
		$this->assertSame('http://example.com', (string)$uri);
	}
	...
	public function testPortIsNullIfStandardPortForScheme() {
		// HTTPS standard port
		$uri = new Url('https://example.com:443');
		$this->assertNull($uri->getPort());
		$this->assertSame('example.com', $uri->getAuthority());
		$uri = (new Url('https://example.com'))->withPort(443);
		$this->assertNull($uri->getPort());
		$this->assertSame('example.com', $uri->getAuthority());
		// HTTP standard port
		$uri = new Url('http://example.com:80');
		$this->assertNull($uri->getPort());
		$this->assertSame('example.com', $uri->getAuthority());
		$uri = (new Url('http://example.com'))->withPort(80);
		$this->assertNull($uri->getPort());
		$this->assertSame('example.com', $uri->getAuthority());
	}

	public function testPortIsReturnedIfSchemeUnknown() {
		$uri = (new Url('//example.com'))->withPort(80);
		$this->assertSame(80, $uri->getPort());
		$this->assertSame('example.com:80', $uri->getAuthority());
	}

	public function testStandardPortIsNullIfSchemeChanges() {
		$uri = new Url('http://example.com:443');
		$this->assertSame('http', $uri->getScheme());
	...
	public function testPortPassedAsStringIsCastedToInt() {
		$exception = null;
		try {
			(new Url('//example.com'))->withPort("8080");
		}
	...
	public function testPortCanBeRemoved() {
		$uri = (new Url('http://example.com:8080'))->withPort(null);
		$this->assertNull($uri->getPort());
		$this->assertSame('http://example.com', (string)$uri);
	}
	...
	public function testAuthorityWithUserInfoOrPortButWithoutHost() {
		$uri = (new Url())->withUserInfo('user', 'pass');
		$this->assertSame('user:pass', $uri->getUserInfo());
	...
	public function testFileSchemeWithEmptyHostReconstruction() {
		$uri = new Url('file:///tmp/filename.ext');
	...
	public function testWithPathEncodesProperly() {
		$uri = (new Url())->withPath('/baz?#€/b%61r');
	...
	public function testWithFragmentEncodesProperly() {
		$uri = (new Url())->withFragment('#€?/b%61r');
	...
	public function testRelativePathAndAuhorityIsAutomagicallyFixed() {
		...
		$uri = (new Url)->withPath('foo')->withHost('example.com');
		$this->assertSame('/foo', $uri->getPath());
	...
	public function testPathStartingWithTwoSlashesAndNoAuthorityIsInvalid() {
		...
		(new Url)->withPath('//foo');
	}

	public function testPathStartingWithTwoSlashes() {
		$uri = new Url('http://example.org//path-not-host.com');
	...
	public function testRelativeUriWithPathBeginngWithColonSegmentIsInvalid() {
		...
		(new Url)->withPath('mailto:foo');
	}

	public function testRelativeUriWithPathHavingColonSegment() {
		$uri = (new Url('urn:/mailto:foo'))->withScheme('');
		$this->assertSame('/mailto:foo', $uri->getPath());
		self::expectException(\InvalidArgumentException::class);
		(new Url('urn:mailto:foo'))->withScheme('');
	}
	...
	public function testGetQueryValue() {
		$uri = new Url("example.com/test?name=cody&colour=orange");
